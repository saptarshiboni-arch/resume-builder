...
export default function PreviewPage({
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 pb-24">
      ...
      <div className="sticky top-16 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 py-3.5 px-4 sm:px-8 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/90 p-5 shadow-sm space-y-3.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-slate-900 text-sm font-display">Choose Resume Style</h2>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`p-3.5 rounded-xl border text-left transition-all relative ${
                        isSelected
                          ? 'border-indigo-600 bg-indigo-50/40 ring-2 ring-indigo-600/30 shadow-sm'
            ...
            <div className="bg-white rounded-2xl border border-slate-200/90 p-5 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm font-display">Quick Section Tweaks</h3>
                </div>
                <span className="text-[10px] text-slate-400 font-medium">Click to refine</span>
              </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                {resumeData?.summary && (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2.5 rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30 transition-all text-left group"
                  >
                    <div>
                      <span className="font-semibold text-slate-800 block">Professional Summary</span>
                      <span className="text-[10px] text-slate-500 line-clamp-1">{resumeData.summary}</span>
                    </div>
                    <Edit3 className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-600 shrink-0 ml-2" />
                  </button>
                )}
                ...
                {resumeData?.projects && resumeData.projects.map((proj, idx) => (
                  ...
                  >
                    <div>
                      <span className="font-semibold text-slate-800 block truncate">Project: {proj.name}</span>
                      <span className="text-[10px] text-slate-500 line-clamp-1">{proj.description}</span>
                    </div>
                    <Edit3 className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-600 shrink-0 ml-2" />
                  </button>
                ))}
                ...
                {resumeData?.experience && resumeData.experience.map((exp, idx) => (
                  <div key={idx} className="space-y-1">
                    <span className="text-[11px] font-bold text-slate-600 px-1">{exp.company} — {exp.jobTitle}</span>
                    {exp.bullets && exp.bullets.map((b, bIdx) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-2 rounded-lg border border-slate-100 bg-slate-50/50 hover:border-indigo-200 hover:bg-indigo-50/20 transition-all text-left group"
                      >
                        <span className="text-[10px] text-slate-600 line-clamp-1">{b}</span>
                        <Edit3 className="w-3 h-3 text-slate-400 group-hover:text-indigo-600 shrink-0 ml-2" />
                      </button>
                    ))}
                  </div>
                ))}
          ...
          <div className="lg:col-span-8">
            <div className="bg-slate-200/80 p-3 sm:p-6 rounded-3xl border border-slate-300 shadow-inner overflow-x-auto">
